...
int writeimage(char *file, uint8_t* image, uint32_t len)
{
	FILE *outf = NULL;

	if (!(outf = fopen(file, "wb+")))
	{
		fprintf(stderr, "Unable to open file %s for output\n", file);
		goto error;
	}

	if (fwrite(image, sizeof(uint8_t), len, outf) != len) 
	{
...
int main(int argc, char **argv)
{
	...
	if (argc != 2)
    {
		printf("usage: imageExtract image\n");
		return 1;
	}
    ...
    if (image && !(imageFile = fopen(image, "rb"))) 
    {
        fprintf(stderr, "Unable to open image file \"%s\"\n", image);
        return 1;
    }

	struct imagetag imageTag;

	if (sizeof(imageTag) != fread(&imageTag, sizeof(uint8_t), sizeof(imageTag), imageFile))
	{
		fprintf(stderr, "Unable to read image file header\n");
		goto error;
	}

	if (strcmp(imageTag.sig1, DEFAULT_SIGNATURE1) != 0)
	{
		fprintf(stderr, "Signature doesn't match expected\n");
		goto error;
	}

	parsetag(imageTag);

	if (imageTag.bigendian[0] == '1')
	{
		imageTag.imagecrc = bswap_32(imageTag.imagecrc);
		imageTag.kernelcrc = bswap_32(imageTag.kernelcrc);
		imageTag.rootfscrc = bswap_32(imageTag.rootfscrc);
	}

	uint32_t cfeImageLen;
	uint32_t rootImageLen;
	uint32_t kernelImageLen;
	rootImageLen = strtoul(imageTag.rootfs.len, NULL, 10);
	cfeImageLen = strtoul(imageTag.cfe.len, NULL, 10);
	kernelImageLen = strtoul(imageTag.kernel.len, NULL, 10);

	uint8_t *pCfeImage = malloc(cfeImageLen);
	uint8_t *pRootImage = malloc(rootImageLen);
	uint8_t *pKernelImage = malloc(kernelImageLen);

	if (cfeImageLen)
	{
		if (cfeImageLen != fread(pCfeImage, sizeof(uint8_t), cfeImageLen, imageFile))
		{
			fprintf(stderr, "Error reading CFE image");
			goto error;
		}
	}
	...
	if (rootImageLen)
	{
		if (rootImageLen != fread(pRootImage, sizeof(uint8_t), rootImageLen, imageFile))
		{
			fprintf(stderr, "Error reading root image");
			goto error;
		}
	}
	...
	if (kernelImageLen)
	{
		if (kernelImageLen != fread(pKernelImage, sizeof(uint8_t), kernelImageLen, imageFile))
		{
			fprintf(stderr, "Error reading kernel image");
			goto error;
		}
	}

	uint32_t imageCrc, kernelCrc, rootCrc;
	imageCrc = kernelCrc = rootCrc = CRC_START;

	imageCrc = crc32(imageCrc, pCfeImage, cfeImageLen);

	rootCrc = crc32(rootCrc, pRootImage, rootImageLen);
	imageCrc = crc32(imageCrc, pRootImage, rootImageLen);

	kernelCrc = crc32(kernelCrc, pKernelImage, kernelImageLen);
	imageCrc = crc32(imageCrc, pKernelImage, kernelImageLen);

	int error = 0;

	if (imageCrc != imageTag.imagecrc)
	{
		fprintf(stderr, "image CRC doesn't match %08lx %08lx\n", imageCrc, imageTag.imagecrc);
		error = 1;
	}

	if (rootCrc != imageTag.rootfscrc)
	{
		fprintf(stderr, "root fs doesn't match %08lx %08lx\n", rootCrc, imageTag.rootfscrc);
		error = 1;
	}

	if (kernelCrc != imageTag.kernelcrc)
	{
		fprintf(stderr, "kernel CRC doesn't match %08lx %08lx\n", kernelCrc, imageTag.kernelcrc);
		error = 1;
	}
	...
	writeimage("rootfs.img", pRootImage, rootImageLen);
	writeimage("cfe.img", pCfeImage, cfeImageLen);
	writeimage("kernel.img", pKernelImage, kernelImageLen);
...
int foo(int argc, char **argv)
{
    ...
    static char usage_msg[] = "Usage: imageExtract image\n";

    image=kernel=rootfs=cfe=outfile=boardid=chipid=signature1=signature2=tagver=vendorid=compileid=NULL;
